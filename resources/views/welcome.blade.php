...
.trust-sub { color: var(--text-secondary); font-size: 11px; }
  ...
  .slide-1 { background: linear-gradient(135deg, #FFBBC0 0%, #FFEEEE 60%, #FFBBC0 100%); }
  .slide-2 { background: linear-gradient(135deg, #fde0d8ff 0%, #ffcabeff 60%, #ff8d70ff 100%); }
  .slide-3 { background: linear-gradient(135deg, #dbf5e0ff 0%, #9dc69cff 60%, #598f4eff 100%); }

  .slide-content {
    position: absolute; left: 8%; bottom: 60px;
    z-index: 2;
  }
  .slide-label {
    font-size: 11px; font-weight: 600; letter-spacing: 2px;
    color: var(--primary-deeper); text-transform: uppercase; margin-bottom: 10px;
  }
  .slide-title {
    font-size: 42px; font-weight: 800; color: var(--text);
    line-height: 1.15; margin-bottom: 16px; max-width: 480px;
  }
  .slide-3 .slide-label{
    color: #2D5016; /* nulis ulang khusus slide 1 */
  }
...
.slide-3 .btn-slide{
  ...
  .slide-sub {
    ...
    margin-bottom: 28px; max-width: 380px; line-height: 1.6;
  }
  .btn-slide {
  ...
  .dot {
    ...
    background: rgba(93,57,59,0.25); cursor: pointer; transition: all 0.3s;
  }
...
.trust-bar {
    ...
    gap: 20px;
    padding: 32px 48px; /* ← jarak dari pinggir kiri-kanan */
    background: white;
    margin: 0px 48px;
  }
  .trust-icon{
    font-size: 28px; color: var(--primary-deeper); font-weight: 700;
  }
  .trust-item {
  ...
  .category-grid {
    ...
    gap: 14px;
    max-width: 900px; /* ← tambahin ini */
    margin: 5px auto; /* ← tambahin ini untuk center */
  }

  .category-card {
    position: relative; overflow: hidden; border-radius: 16px; max-height: 400px;
  ...
  .category-card img {
    ...
    position: absolute;
    inset: 0;
    max-height: 400px; /* ← batasi tinggi fotonya */
}
  .category-card:hover { transform: translateY(-4px); box-shadow: 0 16px 40px rgba(255,187,192,0.3); }
  .category-card.wide { grid-column: span 2; aspect-ratio: unset; min-height: 260px; }
  .category-icon { font-size: 52px; }
  .category-label {
    font-size: 13px; font-weight: 600; color: var(--text); letter-spacing: 0.5px;
  }
  .category-sub { font-size: 11px; color: var(--text-secondary); }
  .category-overlay {
    position: absolute; inset: 0; background: rgba(93,57,59,0);
  ...
  .category-hover-label {
    ...
    gap: 6px; /* ← jarak antara label dan sub */
    transition: all 0.3s;
    border-radius: 16px;
}
.category-hover-sub {
...
.category-hover-label::before {
    content: attr(data-sub); /* ← pake ::before buat sub */
    font-size: 11px;
    font-weight: 400;
    opacity: 0;
    transition: opacity 0.3s;
    order: 2; /* ← taruh di bawah label */
}

.category-hover-sub::after {
...
.category-card:hover .category-hover-label {
    background: rgba(93, 57, 59, 0.45);
}


.category-card:hover .category-hover-label::after,
.category-card:hover .category-hover-sub::after {
    opacity: 1;
}
  ...
  /* ── PRODUCT SECTION ── */
  .product-section { padding: 60px 48px; }
  .prod-header {
    display: flex; justify-content: space-between; align-items: baseline;
    margin-bottom: 32px;
  }
  .prod-title { font-size: 22px; font-weight: 700; color: var(--text); }
  .view-all-link {
    font-size: 13px; font-weight: 500; color: var(--text-secondary);
  ...
  .product-img-wrap {
    ...
    background: var(--surface); aspect-ratio: 3/4; margin-bottom: 14px;
    display: flex; align-items: center; justify-content: center;
  ...
  .product-img-wrap .p-icon { font-size: 52px; }
  .product-img-wrap .p-label { font-size: 11px; color: var(--text-secondary); }
  .badge-v {
  ...
  .p-brand { font-size: 11px; color: var(--text-secondary); margin-bottom: 4px; }
  .p-name { font-size: 14px; font-weight: 600; color: var(--text); margin-bottom: 6px; line-height: 1.4; }
  .p-price { font-size: 15px; font-weight: 700; color: var(--text); }
  .p-original { font-size: 12px; font-weight: 400; color: var(--text-secondary); text-decoration: line-through; margin-left: 6px; }
  .p-discount { font-size: 11px; font-weight: 600; color: var(--success); margin-left: 6px; }
  ...
  .btn-eco { background: var(--success); color: white; border: none; padding: 12px 32px; border-radius: 100px; font-family: 'Poppins', sans-serif; font-size: 14px; font-weight: 600; cursor: pointer; transition: all 0.2s; }
...
<nav>
  <div class="nav-top">
    ...
    <div class="nav-right">
      <a href="{{ $loginUrl }}" class="btn-login">Masuk</a>
    </div>
  </div>
</nav>
...
<div class="product-section" id="produk">
  <div class="prod-header">
    ...
    <a href="{{ $loginUrl }}" class="view-all-link">Lihat Semua →</a>
  </div>
  <div class="product-grid">
    <div class="product-card" onclick="window.location='{{ $loginUrl }}'">
      <div class="product-img-wrap">
        <img src="https://images.unsplash.com/photo-1580870069867-74c57ee1bb07?q=80&w=735&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"/>
        <span class="badge-v">✓ Verified</span>
        <div class="wishlist-btn">🤍</div>
        <button class="btn-cart-hover" onclick="event.stopPropagation();window.location='{{ $loginUrl }}'">+ KERANJANG</button>
      </div>
    ...
    <div class="product-card" onclick="window.location='{{ $loginUrl }}'">
      <div class="product-img-wrap">
        <img src="https://images.unsplash.com/photo-1586495777744-4413f21062fa?q=80&w=715&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"/>
        <span class="badge-v">✓ Verified</span>
        <div class="wishlist-btn">🤍</div>
        <button class="btn-cart-hover" onclick="event.stopPropagation();window.location='{{ $loginUrl }}'">+ KERANJANG</button>
      </div>
    ...
    <div class="product-card" onclick="window.location='{{ $loginUrl }}'">
      <div class="product-img-wrap">
        <img src="https://plus.unsplash.com/premium_photo-1670514094627-9ed5a1e9c06f?q=80&w=686&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"/>
        <span class="badge-v">✓ Verified</span>
        <div class="wishlist-btn">🤍</div>
        <button class="btn-cart-hover" onclick="event.stopPropagation();window.location='{{ $loginUrl }}'">+ KERANJANG</button>
      </div>
    ...
    <div class="product-card" onclick="window.location='{{ $loginUrl }}'">
      <div class="product-img-wrap">
        <img src="https://plus.unsplash.com/premium_photo-1677175230595-87f8721ff703?q=80&w=784&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"/>
        <span class="badge-v">✓ Verified</span>
        <div class="wishlist-btn">🤍</div>
        <button class="btn-cart-hover" onclick="event.stopPropagation();window.location='{{ $loginUrl }}'">+ KERANJANG</button>
      </div>
